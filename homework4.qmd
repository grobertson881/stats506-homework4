...
*Part B*
```{r}
### This isn't working, may have to reformat
...
highest_state_data <- covid %>% 
  filter(state == data_by_state$state[1])

lowest_state_data <- covid %>% 
  filter(state == data_by_state$state[nrow(data_by_state)])

ggplot(highest_state_data, aes(x = date, y = cases_avg_per_100k)) + 
  geom_line(color = "American Samoa") +
  geom_line(data = lowest_state_data, color = "Maryland") +
  scale_color_manual(
    name = "State",
    values = c("Maryland" = "blue", "American Samoa" = "red")
  ) +
  labs(
    x = "Date",
    y = "Average New Cases per 100k People",
    title = "COVID-19 Average Daily New Cases by Population",
    subtitle = "Only States with Highest and Lowest Rates Shown",
    caption = "Source: NYT COVID Data"
  )
